...
export default function CreateEventPage() {
  ...
  async function onSubmit(data: CreateEventInput) {
    ...
    try {
      ...
      if (!pricingRes.ok) {
        setError(pricingJson?.error || 'Não foi possível salvar o valor da inscrição')
        return
      }

      // Redirecionar para a tela do evento criado
      router.push(`/eventos/${result.event.id}?created=1`)
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-slate-900">Criar Novo Evento</h1>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="Ex: Workshop de React 2024"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
